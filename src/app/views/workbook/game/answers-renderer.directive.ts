...
export class AnswersRendererDirective {  
  ...
  private _initialize(answers: string[]): void {
    ...
    for (let index = 0; index < sortedAnswersLen; index++) {
      ...
      rowEl.classList.value = 'wg-answer-row';

      /**
       * <div class="wg-answer-row">
       *   <div class="wg-answer-overlay"></div>
       *   <div>{{ answers[i] }}</div>
       * </div>
       */
      for (let i = 0; i < 2; i++) {
        ...
        if (answerHTML) {
          ...
          answerEl.classList.value = 'wg-answer';
          ...
          overlayEl.classList.value = 'wg-answer-overlay';
          answerEl.appendChild(overlayEl);
    
          const contentEl = docRef.createElement('div');
          contentEl.classList.value = 'wg-answer-content wg-typography';
